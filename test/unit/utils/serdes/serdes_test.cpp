...
namespace
{
    ...
    void testString()
    {
        ...
        // Test long tag and long string value.
        {
            ...
            const std::string out = ser.getBuffer();
            const std::string ref = std::string("N1XL") + "\xff" + char(str.size() & 0xff) + str + "\xff" + char(str.size() & 0xff) + str;
            assert(out == ref);
            nixlSerDes ser2;
            ser2.addStr(str, str);
            const std::string out2 = ser2.exportStr();
    ...
    template<typename T>
    void testIntegral(const T val, const std::string& ref)
    {
