...
namespace nixl
{
    ...
    void serializer::addLongLength(size_t len)
    {
        uint8_t tmp[9];
        uint8_t used = 0;
    ...
    void serializer::addRawImpl(const std::string_view& str)
    {
        addRawImpl(str.data(), str.size());
    }

    void serializer::addRawImpl(const void* buf, const size_t len)
    {
    ...
    void serializer::append(const char c)
    {
        buffer_ += c;
    }

    void serializer::append(const void* buf, const size_t len)
    {
        ...
        : buffer_(static_cast<const char*>(data), size)
    {
        ...
        if(std::memcmp(buffer_.data(), introString.data(), introString.size()) != 0) /* [[unlikely]] -- C++20 */ {
            throw std::runtime_error("invalid deserialization intro data");
        }
        consume(introString.size());
    }

    nixl_status_t deserializer::getString(const std::string_view& tag, std::string& result)
    {
        if(findTag(tag)) {
    ...
    nixl_status_t deserializer::getBinary(const std::string_view& tag, void* data, size_t& size)
    {
        if(findTag(tag)) {
    ...
    void deserializer::require(const size_t amount) const
    {
    ...
    void deserializer::consume(const size_t amount) noexcept
    {
        // assert(buffer_.size() >= amount);
        buffer_ = buffer_.substr(amount);
    }

    size_t deserializer::getTagLength()
    {
    ...
    size_t deserializer::getLongLength(const size_t bytes)
    {
        require(bytes);
        size_t result = 0;
    ...
    std::string_view deserializer::getStringView()
    {
    ...
    int8_t deserializer::getFirstByte()
    {
        require(1);
    ...
    bool deserializer::findTag(const std::string_view& tag)
    {
    ...
    nixl_status_t deserializer::getIntegralImpl1(const int8_t first, void* result, const size_t size)
    {
        require(first);
    ...
    nixl_status_t deserializer::getIntegralImpl2(const int8_t first, void* result, const size_t size)
    {
        switch(first & 0xb0) {
            case 0x80:
                if(size != 1 ) {
    ...
    nixl_status_t deserializer::getIntegralImpl(void* result, const size_t size)
    {
        const int8_t first = getFirstByte();
    ...
    namespace internal
    {
        namespace
        {
            [[nodiscard]] constexpr size_t compressedSize(const int8_t first, const size_t fallback) noexcept
            {
                 switch(first & 0xb0) {
                     case 0x80:
                         return 1;
        ...
        ssize_t compatibility::getBufLenCompat(compatibility& des)
        {
        ...
        ssize_t compatibility::getBufLenCompat(const std::string_view& tag)
        {
        ...
        nixl_status_t compatibility::getBufCompat(void* buf, const ssize_t size)
        {
            const int8_t first = getFirstByte();
        ...
        nixl_status_t compatibility::getBufCompat(const std::string_view& tag, void* buf, const ssize_t size)
        {
            compatibility des(*this);
        ...
        std::string compatibility::getStrCompat()
        {
            const int8_t first = getFirstByte();
        ...
        std::string compatibility::getStrCompat(const std::string_view& tag)
        {
            compatibility des(*this);
        ...
        string_holder::string_holder(std::string&& data) noexcept
    ...
    namespace
    {
        ...
        [[nodiscard]] Int copyFromRaw(const void* buf)
        {
            Int result;
...
nixl::serializer& nixlSerDes::serializer()
{
    return std::get<0>(impl_);
}

const nixl::serializer& nixlSerDes::serializer() const
{
    return std::get<0>(impl_);
}

nixl::internal::deserializer& nixlSerDes::deserializer()
{
    return std::get<1>(impl_);
}

const nixl::internal::deserializer& nixlSerDes::deserializer() const
{
    return std::get<1>(impl_);
}

nixl::internal::deserializer& nixlSerDes::mutable_deserializer() const
{
    return const_cast<nixl::internal::deserializer&>(std::get<1>(impl_));
}

nixlSerDes::nixlSerDes()
    : impl_(std::in_place_type_t<nixl::serializer>())
{}
...
nixl_status_t nixlSerDes::getBuf(const std::string &tag, void *buf, const ssize_t len) {
...
nixl_status_t nixlSerDes::importImpl(std::string&& data) {
    try {
...
nixl_status_t nixlSerDes::importStr(const std::string& data) {
