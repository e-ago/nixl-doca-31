...
std::string
nixlEnumStrings::xferOpStr (const nixl_xfer_op_t &op) {
...
#endif // HAVE_ETCD
    if (name.empty()) throw std::invalid_argument ("Agent needs a name");
...
nixlAgent::nixlAgent (const std::string &name, const nixlAgentConfig &cfg)
    ...
    if (data->useEtcd || cfg.useListenThread) {
        ...
        data->commThread = std::thread (&nixlAgentData::commWorker, data.get(), this);
    }
}
...
nixlAgent::createBackend (const nixl_backend_t &type,
    ...
    if (data->backendEngines.count (type) != 0) return NIXL_ERR_INVALID_PARAM;
    ...
    if (backend) {
        ...
        if (backend->supportsRemote()) {
            if (!backend->supportsNotif()) {
                NIXL_ERROR << "Backend " << type
                           << " supports remote transfer but not notifications";
                delete backend;
                return NIXL_ERR_BACKEND;
            }
...
nixlAgent::registerMem (const nixl_reg_dlist_t &descs, const nixl_opt_args_t *extra_params) {
    ...
    if (!extra_params || extra_params->backends.size() == 0) {
        ...
        if (backend_list->empty()) return NIXL_ERR_NOT_FOUND;
    } else {
    ...
    for (size_t i = 0; i < backend_list->size(); ++i) {
        ...
        if (ret == NIXL_SUCCESS) {
            if (backend->supportsLocal()) {
                ...
                ret = data->remoteSections[data->name]->loadLocalData (sec_descs, backend);
                if (ret == NIXL_SUCCESS)
                    count++;
                else
    ...
    if (extra_params && extra_params->backends.size() > 0) delete backend_list;
...
nixlAgent::deregisterMem (const nixl_reg_dlist_t &descs, const nixl_opt_args_t *extra_params) {
    ...
    if (!extra_params || extra_params->backends.size() == 0) {
        ...
        if (!avail_backends || avail_backends->empty()) return NIXL_ERR_NOT_FOUND;
...
nixlAgent::makeConnection (const std::string &remote_agent, const nixl_opt_args_t *extra_params) {
    ...
    NIXL_LOCK_GUARD (data->lock);
    if (data->remoteBackends.count (remote_agent) == 0) return NIXL_ERR_NOT_FOUND;

    if (!extra_params || extra_params->backends.size() == 0) {
        if (data->remoteBackends[remote_agent].empty()) return NIXL_ERR_NOT_FOUND;
...
nixlAgent::prepXferDlist (const std::string &agent_name,
    ...
    if (!init_side && (data->remoteSections.count (agent_name) == 0)) return NIXL_ERR_NOT_FOUND;

    if (!extra_params || extra_params->backends.size() == 0) {
        if (!init_side)
            backend_set = data->remoteSections[agent_name]->queryBackends (descs.getType());
        else
            backend_set = data->memorySection->queryBackends (descs.getType());

        if (!backend_set || backend_set->empty()) return NIXL_ERR_NOT_FOUND;
    } else {
    ...
    for (auto &backend : *backend_set) {
        handle->descs[backend] = new nixl_meta_dlist_t (descs.getType(), descs.isSorted());
        if (init_side)
    ...
    if (extra_params && extra_params->backends.size() > 0) delete backend_set;
...
nixlAgent::makeXferReq (const nixl_xfer_op_t &operation,
    ...
    if (!local_side || !remote_side) return NIXL_ERR_INVALID_PARAM;

    if ((!local_side->isLocal) || (remote_side->isLocal)) return NIXL_ERR_INVALID_PARAM;
    ...
    handle->initiatorDescs = new nixl_meta_dlist_t (local_descs->getType(), false, desc_count);

    handle->targetDescs = new nixl_meta_dlist_t (remote_descs->getType(), false, desc_count);
...
nixlAgent::createXferReq (const nixl_xfer_op_t &operation,
    ...
    if (local_descs.descCount() != remote_descs.descCount()) return NIXL_ERR_INVALID_PARAM;
    for (int i = 0; i < local_descs.descCount(); ++i)
        if (local_descs[i].len != remote_descs[i].len) return NIXL_ERR_INVALID_PARAM;
    ...
    nixlXferReqH *handle = new nixlXferReqH;
    handle->initiatorDescs = new nixl_meta_dlist_t (local_descs.getType(), local_descs.isSorted());

    handle->targetDescs = new nixl_meta_dlist_t (remote_descs.getType(), remote_descs.isSorted());
...
nixlAgent::releaseXferReq (nixlXferReqH *req_hndl) const {
    ...
    if (req_hndl->status == NIXL_IN_PROG) {
        ...
        if (req_hndl->status == NIXL_IN_PROG) {
            ...
            if (req_hndl->status < 0) return NIXL_ERR_REPOST_ACTIVE;
...
nixlAgent::getNotifs (nixl_notifs_t &notif_map, const nixl_opt_args_t *extra_params) {
    ...
    if (!extra_params || extra_params->backends.size() == 0) {
        ...
        if (backend_list->empty()) return NIXL_ERR_BACKEND;
    } else {
        backend_list = new backend_list_t();
        for (auto &elm : extra_params->backends)
            if (elm->engine->supportsNotif()) backend_list->push_back (elm->engine);
    ...
    for (auto &eng : *backend_list) {
        ...
        if (ret < 0) bad_ret = ret;

        if (bknd_notif_list.size() == 0) continue;

        for (auto &elm : bknd_notif_list) {
            if (notif_map.count (elm.first) == 0) notif_map[elm.first] = std::vector<nixl_blob_t>();
...
nixlAgent::genNotif (const std::string &remote_agent,
    ...
    for (auto &eng : *backend_list) {
        if ((localNotif && eng->supportsLocal()) ||
            (!localNotif && data->remoteBackends[remote_agent].count (eng->getType()) != 0)) {
...
nixlAgent::getLocalPartialMD (const nixl_reg_dlist_t &descs,
    ...
    for (const auto &backend : *backend_list) {
        ...
        if (it == data->connMD.end()) continue;
    ...
    if (selected_engines.size() == 0 && descs.descCount() > 0) return NIXL_ERR_BACKEND;
...
nixlAgent::loadRemoteMD (const nixl_blob_t &remote_metadata, std::string &agent_name) {
    ...
    if (remote_agent.size() == 0) return NIXL_ERR_MISMATCH;

    if (remote_agent == data->name) return NIXL_ERR_INVALID_PARAM;
    ...
    for (size_t i = 0; i < conn_cnt; ++i) {
        ...
        if (nixl_backend.size() == 0) return NIXL_ERR_MISMATCH;
        conn_info = sd.getStr ("c");
        if (conn_info.size() == 0) return NIXL_ERR_MISMATCH;
        ...
        if (data->backendEngines.count (nixl_backend) != 0) {
            ...
            if (eng->supportsRemote()) {
                ...
                if (ret) return ret; // Error in load
                count++;
    ...
    if (count == 0 && conn_cnt > 0) return NIXL_ERR_BACKEND;

    if (sd.getStr ("") != "MemSection") return NIXL_ERR_MISMATCH;
...
nixlAgent::sendLocalMD (const nixl_opt_args_t *extra_params) const {
    ...
    if (extra_params && !extra_params->ipAddr.empty()) {
        data->enqueueCommWork (std::make_tuple (
            SOCK_SEND, extra_params->ipAddr, extra_params->port, std::move (myMD)));
        return NIXL_SUCCESS;
    }
